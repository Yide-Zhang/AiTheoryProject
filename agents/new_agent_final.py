...
class SafeActionGenerator:
    """Generates safety-first action sets when eight-ball protection is needed."""
...
class BalancedActionPlanner:
    """Produces risk-aware candidate sets and merges them with base actions."""
...
class NewAgentFinal(Agent):
    """All-in-one fast hybrid agent with balanced risk planning."""
    ...
    def decision(
        ...
        if self._needs_eight_protection(balls, my_targets):
            safeguard = self._safe_mode_decision(balls, my_targets, table)
        ...
        print(
            f"[NewAgentFinal] strategy={best_info.get('strategy')} target={best_info.get('target')} "
            f"pocket={best_info.get('pocket')} plan={self._last_strategy} score={best_score:.1f}"
        )
        return best_action
    ...
    def _generate_candidates(
        ...
        base_candidates = self._build_base_candidates(balls, my_targets, table, phase)
        ...
        print(
            f"[NewAgentFinal] merge strategy={strategy} easy={game_state.easy_shots} "
    ...
    def _safe_mode_decision(
        ...
        if self._is_aiming_directly_at_eight(best_action, balls):
            return self._fallback_defensive_action(balls, table)

        print(
            f"[NewAgentFinal] safe-mode strategy={best_info.get('strategy')} "
            f"target={best_info.get('target')} score={best_score:.1f}"
        )
        return best_action
    ...
    def _score_shot(self, shot: pt.System, last_state: BallDict, my_targets: List[str]) -> float:
        ...
        if self._premature_eight_pocketed(shot, last_state, my_targets):
            score -= max(0.0, self.premature_eight_penalty - 420.0)
